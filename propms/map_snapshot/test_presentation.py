...
class TestMapSnapshotPresentation(PropertyInstructionTestMixin, FrappeTestCase):
	def test_crop_constant_and_presentation_version_are_fixed(self):
		self.assertEqual(MY_MAPS_HEADER_CROP_PX, 68)
		self.assertEqual(MY_MAPS_BOTTOM_OVERSCAN_PX, 2)
		self.assertEqual(MY_MAPS_PRESENTATION_VERSION, 2)
	...
	def test_standard_maps_descriptor_has_no_crop(self):
		...
		self.assertEqual(presentation.bottom_overscan_px, 0)

	def test_my_maps_descriptor_has_geometry(self):
		presentation = get_map_presentation("google-my-maps")
		self.assertEqual(presentation.crop_px, 68)
		self.assertEqual(presentation.bottom_overscan_px, 2)
		self.assertTrue(is_google_my_maps(presentation.map_kind))

	def test_my_maps_source_hash_changes_when_geometry_changes(self):
		...
		with patch(
			...
			return_value=replace(
				get_map_presentation("google-my-maps"),
				crop_px=67,
				bottom_overscan_px=1,
			),
		):
	...
	def test_standard_maps_source_hash_is_unchanged_by_my_maps_only_geometry(self):
		url = "https://www.google.com/maps/embed?pb=abc"
		baseline = build_map_source_hash(url, "google-maps")
		with patch(
			"propms.map_snapshot.presentation.MY_MAPS_PRESENTATION",
			replace(get_map_presentation("google-my-maps"), crop_px=67, bottom_overscan_px=1),
		):
			self.assertEqual(baseline, build_map_source_hash(url, "google-maps"))
	...
	def test_main_my_maps_live_html_contains_crop_class(self):
		...
		self.assertIn("--pi-my-maps-header-crop: 68px", html)
		self.assertIn("--pi-my-maps-bottom-overscan: 2px", html)
		self.assertIn("data-my-maps-presentation-version=\"2\"", html)
		self.assertNotIn("transform: translateY(calc(-1 * var(--pi-my-maps-header-crop)))", html)
	...
	def test_block_my_maps_html_contains_crop_class(self):
		...
		self.assertIn("height: clamp(320px, 52vw, 420px);", html)
