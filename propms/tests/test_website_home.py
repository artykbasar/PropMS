...
class TestWebsiteHome(IntegrationTestCase):
    ...
    def test_theme_drives_brand_tokens_and_card_hover(self):
        ...
        self.assertIn("height: 100lvh;", styles)
        self.assertNotIn("background-attachment: fixed", styles)
        self.assertIn('<meta name="theme-color" content="{{ shell.design_tokens[\'--brand-secondary\'] | e }}">', base)
        self.assertIn("style=\"background-color: {{ shell.design_tokens['--brand-secondary'] | e }};\"", base)
    ...
    def test_v2_home_uses_native_progressive_enhancement(self):
        ...
        self.assertIn("setBrowserChromeColor", script)
        self.assertIn("root.style.backgroundColor", script)
        self.assertNotIn("--v2-fixed-media", script)
