		...
		doc = frappe.get_doc(
			{
				...
				"address": overrides.pop("address", "10 Example Street"),
				"custom_map_embed_url": overrides.pop("custom_map_embed_url", None),
				"google_maps_url": overrides.pop(
					"google_maps_url",
					"https://www.google.com/maps/place/10+Example+Street",
				),
				"wifi_name": overrides.pop("wifi_name", "Example Guest WiFi"),
...
class TestPropertyInstruction(PropertyInstructionTestMixin, FrappeTestCase):
	...
	def test_property_map_requires_embed_even_when_google_maps_url_is_present(self):
		doc = self.make_instruction(
			custom_map_embed_url="",
			google_maps_url="https://www.google.com/maps/place/10+Example+Street",
		)
		property_map = doc.get_property_map()
		self.assertFalse(property_map.embed_url)
		self.assertFalse(property_map.external_url)
	...
	def test_map_block_accepts_valid_google_embed_and_excludes_raw_html_from_context(self):
		embed_html = '<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2484.0!2d2.345678!3d1.234567!2m3!1f0!2f0!3f0!3m2!1i1024!2i768"></iframe>'
		doc = self.make_instruction(
			instruction_blocks=[
				{
					"section": "Finding the Property",
		...
		self.assertEqual(
			...
			"https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2484.0!2d2.345678!3d1.234567!2m3!1f0!2f0!3f0!3m2!1i1024!2i768",
		)
		grouped_sections = doc.get_grouped_blocks()
		block = grouped_sections[0].blocks[0]
		self.assertEqual(block.block_type, "Map")
		self.assertEqual(block.map_embed_url, "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2484.0!2d2.345678!3d1.234567!2m3!1f0!2f0!3f0!3m2!1i1024!2i768")
			...
			instruction_blocks=[
				{
					...
					"block_type": "Map",
					"title": "Arrival map",
					"custom_map_embed_url": '<iframe src="https://www.google.com/maps?output=embed&q=10+Example+Street"></iframe>',
				},
			]
		)
		grouped_sections = doc.get_grouped_blocks()
	...
	def test_step_block_with_embed_html_renders_a_validated_map(self):
		embed_html = '<iframe src="https://www.google.com/maps/embed?pb=!1m18!2m3!1d1!2d2.345678!3d1.234567!3m2!1i1024!2i768!4f13.1"></iframe>'
		doc = self.make_instruction(
			instruction_blocks=[
				{
					"section": "Check-In",
		...
		self.assertEqual(block.map_embed_url, "https://www.google.com/maps/embed?pb=!1m18!2m3!1d1!2d2.345678!3d1.234567!3m2!1i1024!2i768!4f13.1")
		html = self.render_instruction(doc)
	...
	def test_text_block_with_embed_html_renders_a_validated_map(self):
		embed_html = '<iframe src="https://www.google.com/maps?output=embed&q=1.234567,2.345678"></iframe>'
		doc = self.make_instruction(
			instruction_blocks=[
				{
					"section": "Finding the Property",
		...
		self.assertEqual(block.map_embed_url, "https://www.google.com/maps?output=embed&q=1.234567,2.345678")
	...
	def test_protected_identifier_values_are_marked_notranslate(self):
		doc = self.make_instruction(address="10 Example Street", wifi_name="Example Guest WiFi")
		html = self.render_instruction(doc)
		...
		self.assertIn('translate="no">10 Example Street', html)
		self.assertIn('translate="no">Example Guest WiFi', html)
	...
	def test_export_script_uses_web_flow_vector_jspdf_without_html2canvas(self):
		source = self.get_export_script_source()
		...
		self.assertIn('var PDF_RENDERER_ID = "web-flow-vector";', source)
		...
		self.assertNotIn("adaptive-vector", source)
	...
	def test_rendered_instruction_text_preserves_ordinary_spaces(self):
		...
		self.assertIn("10 Example Street", html)
	...
	def test_export_script_uses_the_web_flow_a4_pdf_layout(self):
		source = self.get_export_script_source()
		html = self.render_instruction(self.make_instruction())
		self.assertIn('var PDF_LAYOUT_VERSION = "2026-08-10-web-flow-a4-v2";', source)
		self.assertIn('var PDF_RENDERER_ID = "web-flow-vector";', source)
		...
		self.assertIn('applyWebFlowSectionRows(exportState);', source)
		self.assertIn('return paginateWebFlowDocument(exportState);', source)
		self.assertIn('balanceWebFlowPageRows(exportState);', source)
		self.assertIn('decorateWebFlowPagesWithBookmarks(exportState);', source)
		self.assertIn('function collectPdfPageSummary(exportPages)', source)
		self.assertIn('window.__propertyInstructionPdfWebFlowLayoutSummary', source)
		self.assertIn('data-guide-progress-copy="continued_suffix"', html)
		self.assertIn('data-pdf-layout-mode="web-flow"', html)
		self.assertNotIn('propms_pdf_layout', source)
		self.assertNotIn('adaptive-vector', source)

	def test_export_script_builds_web_flow_rows_and_orientation_aware_cards(self):
		source = self.get_export_script_source()
		html = self.render_instruction(self.make_instruction())
		self.assertIn('function createWebFlowRow(documentNode, cards, fullWidth)', source)
		self.assertIn('function isWebFlowFullWidthCard(cardNode)', source)
		self.assertIn('function applyWebFlowSectionRows(exportState)', source)
		self.assertIn('pending.length === 3', source)
		self.assertIn('pi-export-row--three-up', source)
		self.assertIn('pi-export-row--two-up', source)
		self.assertIn('pi-export-row--full', source)
		self.assertIn('pi-export-card--web-flow-portrait', source)
		self.assertIn('pi-export-card--web-flow-landscape', source)
		self.assertIn('.pi-export-row--three-up {', html)
	...
	def test_export_script_supports_web_flow_page_fill_and_print_safe_cards(self):
		source = self.get_export_script_source()
		html = self.render_instruction(self.make_instruction())
		self.assertIn('function balanceWebFlowPageRows(exportState)', source)
		self.assertIn('var growthRatio = isLastPage ? 0.32 : 0.55;', source)
		self.assertIn('var absoluteGrowthCap = isLastPage ? 120 : 180;', source)
		self.assertIn('data-pdf-web-flow-filled', source)
		self.assertIn('function collectPdfPageSummary(exportPages)', source)
		self.assertIn('PDF_SPARSE_PAGE_THRESHOLD', source)
		self.assertIn('.pi-export-row--web-flow[data-pdf-web-flow-filled="1"] .pi-export-card--web-flow', html)
		self.assertIn('.pi-export-card--web-flow-text-only .pi-export-card-layout', html)
		self.assertIn('background: #fff;', html)
		self.assertIn('object-fit: contain;', html)
		self.assertIn('overflow: hidden;', html)
		self.assertIn('overflow-wrap: anywhere;', html)
		self.assertNotIn('PDF_ADAPTIVE_', source)
		self.assertNotIn('pi-export-card--adaptive', html)
	...
	def test_export_script_uses_the_vector_pdf_renderer(self):
		...
		self.assertIn('VECTOR_PDF_FONT_FAMILY_ARABIC', source)
		self.assertIn('function ensureVectorPdfFontsRegistered(pdf)', source)
		self.assertIn('function buildVectorLayoutModel(exportState, pdfWidth, pdfHeight)', source)
	...
	def test_export_script_removes_renderer_and_layout_switches(self):
		...
		self.assertNotIn('getRequestedPdfLayoutMode', source)
		self.assertNotIn('adaptive-vector', source)
		self.assertNotIn('renderExportPagesToRasterPdf', source)
	...
	def test_repo_contains_browser_matrix_tooling_for_pdf_validation(self):
		tool_path = os.path.abspath(
			os.path.join(
				os.path.dirname(__file__), "..", "..", "..", "tools", "pdf_validation", "browser_matrix.js"
			)
		)
		with open(tool_path) as tool_file:
			tool_source = tool_file.read()
		self.assertIn("playwright", tool_source)
		self.assertIn('renderer: "web-flow-vector"', tool_source)
	...
	def test_pdf_export_has_one_web_flow_vector_mode(self):
		source = self.get_export_script_source()
		self.assertIn('var PDF_RENDERER_ID = "web-flow-vector";', source)
		...
		self.assertNotIn('propms_pdf_layout', source)
		self.assertNotIn('propms_pdf_renderer', source)
		self.assertNotIn('adaptive', source.lower())
	...
	def test_export_script_prints_qr_actions_and_clickable_maps(self):
		source = self.get_export_script_source()
		html = self.render_instruction(self.make_instruction())
		self.assertIn('overviewMapLink.className = "pi-export-map-image-link"', source)
		self.assertIn('blockMapLink.className = "pi-export-map-image-link pi-export-map-image-link--block"', source)
		self.assertIn('function getPrintableQrActionLabel(kind, fallbackLabel)', source)
		self.assertIn('getGuideCopyText("scan_wifi", "Scan to connect to Wi-Fi")', source)
		self.assertIn('getGuideCopyText("scan_map", "Scan to open map")', source)
		self.assertIn('getGuideCopyText("scan_link", "Scan to open link")', source)
		self.assertIn('data-guide-progress-copy="scan_wifi"', html)
		self.assertIn('data-guide-progress-copy="scan_map"', html)
		self.assertIn('data-guide-progress-copy="scan_link"', html)
		self.assertIn('anchor.getClientRects()', source)
		self.assertIn('.pi-export-map-image-link {', html)
	...
	def test_export_script_has_no_legacy_adaptive_parking_mutation_path(self):
		source = self.get_export_script_source()
		self.assertNotIn('moveAdaptiveParkingActionsToMap', source)
		self.assertNotIn('moveAdaptiveParkingCardActionsToMedia', source)
		self.assertNotIn('propms_pdf_debug_disable_parking_action_relocation', source)
		self.assertNotIn('data-pdf-parking-actions-relocated', source)
		self.assertNotIn('parkingActionRelocations', source)

	def test_export_script_validates_image_ratio_and_clipping_without_adaptive_card_planner(self):
		source = self.get_export_script_source()
		self.assertIn('function validateImageAspectRatios(exportRoot)', source)
		self.assertIn('function validateExportImageClipping(exportRoot)', source)
		self.assertIn('PDF_MEDIA_RATIO_BLOCKER = 0.01', source)
		self.assertIn('Export image aspect ratio changed beyond tolerance', source)
		self.assertIn('export-image-clipped', source)
		self.assertNotIn('adaptive-card-containment-overflow', source)

	def test_export_script_centers_media_inside_filled_web_flow_rows(self):
		source = self.get_export_script_source()
		html = self.render_instruction(self.make_instruction())
		self.assertIn('webFlowMediaColumn.style.alignItems = "center";', source)
		self.assertIn('webFlowMediaColumn.style.justifyContent = "center";', source)
		self.assertIn('.pi-export-card-media {', html)
	...
	def test_resolve_public_pdf_image_target_treats_external_frappe_style_url_as_remote(self):
		url = "https://brand-b.example/files/example%20image.jpeg"
	...
	def test_validate_public_pdf_image_url_allows_registered_external_file_url(self):
		url = "https://brand-b.example/files/test-guide-image.png"
		self.make_external_file_record(url)
		parsed = validate_public_pdf_image_url(url)
		self.assertEqual(parsed.hostname, "brand-b.example")
	...
	def test_export_script_uses_source_order_web_flow_without_candidate_scoring(self):
		source = self.get_export_script_source()
		self.assertIn('pending.push(cardNode);', source)
		self.assertIn('if (pending.length === 3)', source)
		self.assertIn('rowNodes.forEach(function (rowNode, rowIndex)', source)
		self.assertNotIn('createAdaptiveCardCandidates', source)
		self.assertNotIn('selectBestAdaptivePairCandidate', source)
		self.assertNotIn('scoreAdaptivePageCandidate', source)
		self.assertNotIn('adaptive-no-valid-row-plan', source)
