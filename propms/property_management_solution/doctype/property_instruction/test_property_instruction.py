...
class TestPropertyInstruction(PropertyInstructionTestMixin, FrappeTestCase):
	...
	def test_export_script_uses_per_page_html2canvas_and_direct_jspdf(self):
		...
		self.assertIn("function prepareCaptureFramePage(pageNode)", source)
	...
	def test_export_script_resolves_images_through_same_origin_proxy_and_validates_painting(self):
		...
		self.assertIn("function temporarilyInlineDocumentProxyImages(exportRoot, exportImageDataCache)", source)
		self.assertIn("function buildInlineImageDataMap(restoreEntries, inlineImageDiagnostics, exportImageDataCache)", source)
		self.assertIn('document.querySelectorAll("img")', source)
		self.assertIn("exportRoot.contains(img)", source)
		...
		self.assertIn("temporarilyInlineDocumentProxyImages(exportState.exportRoot, exportImageDataCache)", source)
		self.assertIn("data-export-inline-proxy-src", source)
	...
	def test_export_script_records_pdf_performance_and_ignores_non_current_pages(self):
		...
		self.assertIn("captureTarget = await prepareCaptureFramePage(pageNode)", source)
		self.assertIn('node.setAttribute("data-html2canvas-ignore", "true")', source)
		self.assertIn("destroyCaptureFrame(captureFrame)", source)
		self.assertIn("prepareCaptureClone(clonedDocument, inlineImageDataMap)", source)
