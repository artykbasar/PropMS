...
class PropertyPageRenderer(DocumentPage):
    def can_render(self):
        from propms.website_routes import get_public_development_by_path

        if page := get_public_development_by_path(self.path):
...
class Property(NestedSet):
    ...
    def get_context(self, context):
        if self.publish_online != 1:
            raise frappe.PermissionError
        from propms.website_routes import apply_property_context

        apply_property_context(context, self)
        return context
